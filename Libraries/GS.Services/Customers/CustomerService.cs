...
namespace GS.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        public virtual IPagedList<Customer> GetAllCustomers(DateTime? createdFromUtc = null,
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false, int UnitId = 0)
        {
            ...
            if (!string.IsNullOrWhiteSpace(firstName))
            {
                query = query
                    ...
                    .Where(z => z.Attribute.KeyGroup == _entityName &&
                                z.Attribute.Key == GSCustomerDefaults.FirstNameAttribute &&
            ...
            if (!string.IsNullOrWhiteSpace(lastName))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where(z => z.Attribute.KeyGroup == _entityName &&
                                z.Attribute.Key == GSCustomerDefaults.LastNameAttribute &&
            ...
            {
                var _queryAttribute = _gaRepository.Table.Where(c => c.Value.Contains(fullName) &&
                     (c.Key == GSCustomerDefaults.FirstNameAttribute || c.Key == GSCustomerDefaults.LastNameAttribute)).Select(x => x.EntityId);
                query = query.Where(c =>_queryAttribute.Contains(c.Id));
            }
            ...
            if (dayOfBirth > 0 && monthOfBirth > 0)
            {
                ...
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where(z => z.Attribute.KeyGroup == _entityName &&
                                z.Attribute.Key == GSCustomerDefaults.DateOfBirthAttribute &&
            ...
            if (!string.IsNullOrWhiteSpace(company))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where(z => z.Attribute.KeyGroup == _entityName &&
                                z.Attribute.Key == GSCustomerDefaults.CompanyAttribute &&
            ...
            if (!string.IsNullOrWhiteSpace(phone))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where(z => z.Attribute.KeyGroup == _entityName &&
                                z.Attribute.Key == GSCustomerDefaults.PhoneAttribute &&
            ...
            if (!string.IsNullOrWhiteSpace(zipPostalCode))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where(z => z.Attribute.KeyGroup == _entityName &&
                                z.Attribute.Key == GSCustomerDefaults.ZipPostalCodeAttribute &&
            ...
            if (UnitId > 0)
            {
                query = query.Join(_customerCustomerUnitMappingRepository.Table, x => x.Id, y => y.CustomerId,
                        (x, y) => new { Customer = x, Mapping = y })
                    .Where(z => UnitId==z.Mapping.UnitId)
                    .Select(z => z.Customer)
                    .Distinct();
            }
            /*UnitId.Contains(z.Mapping.UnitId)*/
        ...
        public IPagedList<Customer> GetSearchAllCustomer(string Keyword = null, int[] ExcludeCustomerIds = null, int[] customerRoleIds = null, List<int> SelectedUnitIds = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = _customerRepository.Table;
            if (customerRoleIds != null && customerRoleIds.Length > 0)
            {
            ...
            {
                query = query.Where(c => !ExcludeCustomerIds.Contains(c.Id));
            }
            if (!string.IsNullOrEmpty(Keyword))
            {                
                var _queryAttribute = _gaRepository.Table.Where(c => c.Value.Contains(Keyword) &&
                     (c.Key == GSCustomerDefaults.FirstNameAttribute || c.Key == GSCustomerDefaults.LastNameAttribute)).Select(x => x.EntityId);
                query = query.Where(c => c.Username.Contains(Keyword)
                    || _queryAttribute.Contains(c.Id));
            }
            if (SelectedUnitIds != null && SelectedUnitIds.Count >0)
            {
                query = query.Join(_customerCustomerUnitMappingRepository.Table, x => x.Id, y => y.UnitId,
                        (x, y) => new { Customer = x, Mapping = y })
                    .Where(z => SelectedUnitIds.Contains(z.Mapping.UnitId))
                    .Select(z => z.Customer)
                    .Distinct();
            }
            var customers = new PagedList<Customer>(query, pageIndex, pageSize);
        ...
        public virtual void DeleteCustomer(Customer customer)
        {
            ...
            if (customer.IsSystemAccount)
                throw new GSException($"System customer account ({customer.SystemName}) could not be deleted");
        ...
        public virtual Customer InsertGuestCustomer()
        {
            ...
            //add to 'Guests' role
            var guestRole = GetCustomerRoleBySystemName(GSCustomerDefaults.GuestsRoleName);
            if (guestRole == null)
                throw new GSException("'Guests' role could not be loaded");
            ...
            bool clearPaymentMethod = true)
        {
            ...
            if (clearCouponCodes)
            {
                _genericAttributeService.SaveAttribute<string>(customer, GSCustomerDefaults.DiscountCouponCodeAttribute, null);
                _genericAttributeService.SaveAttribute<string>(customer, GSCustomerDefaults.GiftCardCouponCodesAttribute, null);
            }

            //clear checkout attributes
            if (clearCheckoutAttributes)
            {
                _genericAttributeService.SaveAttribute<string>(customer, GSCustomerDefaults.CheckoutAttributes, null, storeId);
            }

            //clear reward points flag
            if (clearRewardPoints)
            {
                _genericAttributeService.SaveAttribute(customer, GSCustomerDefaults.UseRewardPointsDuringCheckoutAttribute, false, storeId);
            }
            ...
            {
                _genericAttributeService.SaveAttribute<string>(customer, GSCustomerDefaults.SelectedPaymentMethodAttribute, null, storeId);
            }
        ...
        public virtual string GetCustomerFullName(Customer customer)
        {
            ...
            var firstName = _genericAttributeService.GetAttribute<string>(customer, GSCustomerDefaults.FirstNameAttribute);
            var lastName = _genericAttributeService.GetAttribute<string>(customer, GSCustomerDefaults.LastNameAttribute);
        ...
        public virtual string FormatUserName(Customer customer, bool stripTooLong = false, int maxLength = 0)
        {
            ...
            switch (_customerSettings.CustomerNameFormat)
            {
                ...
                case CustomerNameFormat.ShowFirstName:
                    result = _genericAttributeService.GetAttribute<string>(customer, GSCustomerDefaults.FirstNameAttribute);
                    break;
                default:
                    break;
            }
        ...
        public virtual string[] ParseAppliedDiscountCouponCodes(Customer customer)
        {
            ...
            var existingCouponCodes = _genericAttributeService.GetAttribute<string>(customer, GSCustomerDefaults.DiscountCouponCodeAttribute);
        ...
        public virtual void ApplyDiscountCouponCode(Customer customer, string couponCode)
        {
            ...
            //apply new value
            _genericAttributeService.SaveAttribute(customer, GSCustomerDefaults.DiscountCouponCodeAttribute, result);
        }
        ...
        public virtual string[] ParseAppliedGiftCardCouponCodes(Customer customer)
        {
            ...
            var existingCouponCodes = _genericAttributeService.GetAttribute<string>(customer, GSCustomerDefaults.GiftCardCouponCodesAttribute);
        ...
        public virtual void ApplyGiftCardCouponCode(Customer customer, string couponCode)
        {
            ...
            //apply new value
            _genericAttributeService.SaveAttribute(customer, GSCustomerDefaults.GiftCardCouponCodesAttribute, result);
        }
        ...
        public virtual Decimal GetTotalCustomer()
        {
            var query = _customerRepository.Table
                .Join(_customerCustomerRoleMappingRepository.Table, x => x.Id, y => y.CustomerId,
                (x,y) => new { customer = x, role = y})
                .Where(c => c.role.CustomerRoleId == 3);
            return query.Count();
        }
        ...
        public virtual void DeleteCustomerRole(CustomerRole customerRole)
        {
            ...
            if (customerRole.IsSystemRole)
                throw new GSException("System role could not be deleted");

            _customerRoleRepository.Delete(customerRole);

            _cacheManager.RemoveByPattern(GSCustomerServiceDefaults.CustomerRolesPatternCacheKey);
        ...
        public virtual CustomerRole GetCustomerRoleBySystemName(string systemName)
        {
            ...
            var key = string.Format(GSCustomerServiceDefaults.CustomerRolesBySystemNameCacheKey, systemName);
        ...
        {
            var key = string.Format(GSCustomerServiceDefaults.CustomerRolesAllCacheKey, showHidden);
            return _cacheManager.Get(key, () =>
            {
                var query = from cr in _customerRoleRepository.Table
                            orderby cr.Name
        ...
        public IList<CustomerRole> GetCustomerRolesByPrefix(string strPre)
        {
           return  _customerRoleRepository.Table.Where(c => c.SystemName.StartsWith(strPre)).ToList();
        }
        #endregion
        ...
        public virtual bool IsPasswordRecoveryTokenValid(Customer customer, string token)
        {
            ...
            var cPrt = _genericAttributeService.GetAttribute<string>(customer, GSCustomerDefaults.PasswordRecoveryTokenAttribute);
        ...
        public virtual bool IsPasswordRecoveryLinkExpired(Customer customer)
        {
            ...
            var geneatedDate = _genericAttributeService.GetAttribute<DateTime?>(customer, GSCustomerDefaults.PasswordRecoveryTokenDateGeneratedAttribute);
        ...
        public virtual bool PasswordIsExpired(Customer customer)
        {
            ...
            var cacheKey = string.Format(GSCustomerServiceDefaults.CustomerPasswordLifetimeCacheKey, customer.Id);
