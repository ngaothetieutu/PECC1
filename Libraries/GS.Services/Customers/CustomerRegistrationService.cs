    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual CustomerRegistrationResult RegisterCustomer(CustomerRegistrationRequest request)
        {
            ...
            if (registeredRole == null)
                throw new GSException("'Registered' role could not be loaded");
            ...
            var guestRole = request.Customer.CustomerRoles.FirstOrDefault(cr => cr.SystemName == GSCustomerDefaults.GuestsRoleName);
            if (guestRole != null)
            {
        ...
        public virtual void SetEmail(Customer customer, string newEmail, bool requireValidation)
        {
            ...
            if (newEmail == null)
                throw new GSException("Email cannot be null");

            newEmail = newEmail.Trim();
            var oldEmail = customer.Email;

            if (!CommonHelper.IsValidEmail(newEmail))
                throw new GSException(_localizationService.GetResource("Account.EmailUsernameErrors.NewEmailIsNotValid"));

            if (newEmail.Length > 100)
                throw new GSException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailTooLong"));

            var customer2 = _customerService.GetCustomerByEmail(newEmail);
            if (customer2 != null && customer.Id != customer2.Id)
                throw new GSException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailAlreadyExists"));
            ...
            if (requireValidation)
            {
                ...
                _genericAttributeService.SaveAttribute(customer, GSCustomerDefaults.EmailRevalidationTokenAttribute, Guid.NewGuid().ToString());
        ...
        public virtual void SetUsername(Customer customer, string newUsername)
        {
            ...
            if (!_customerSettings.UsernamesEnabled)
                throw new GSException("Usernames are disabled");

            newUsername = newUsername.Trim();

            if (newUsername.Length > GSCustomerServiceDefaults.CustomerUsernameLength)
                throw new GSException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameTooLong"));

            var user2 = _customerService.GetCustomerByUsername(newUsername);
            if (user2 != null && customer.Id != user2.Id)
                throw new GSException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameAlreadyExists"));
