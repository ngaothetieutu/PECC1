...
namespace GS.Services.Plugins
{
    ...
    public class UploadService : IUploadService
    {
        ...
        protected readonly IGSFileProvider _fileProvider;
        ...
        protected virtual IList<UploadedItem> GetUploadedItems(string archivePath)
        {
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                ...
                var uploadedItemsFileEntry = archive.Entries
                    .FirstOrDefault(entry => entry.Name.Equals(GSPluginDefaults.UploadedItemsFileName, StringComparison.InvariantCultureIgnoreCase)
        ...
        protected virtual IDescriptor UploadSingleItem(string archivePath)
        {
            ...
            var pluginsDirectory = _fileProvider.MapPath(GSPluginDefaults.Path);

            //get path to the themes directory
            var themesDirectory = string.Empty;
            if (!string.IsNullOrEmpty(GSPluginDefaults.ThemesPath))
                themesDirectory = _fileProvider.MapPath(GSPluginDefaults.ThemesPath);
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                ...
                if (rootDirectories.Count != 1)
                {
                    throw new Exception("The archive should contain only one root plugin or theme directory. " +
                        ...
                        $"To upload multiple items, the archive should have the '{GSPluginDefaults.UploadedItemsFileName}' file in the root");
                }
                ...
                foreach (var entry in archive.Entries)
                {
                    ...
                    var isPluginDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{GSPluginDefaults.DescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);

                    //or whether it's a theme descriptor
                    var isThemeDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{GSPluginDefaults.ThemeDescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);
                    ...
                    using (var unzippedEntryStream = entry.Open())
                    {
                        using (var reader = new StreamReader(unzippedEntryStream))
                        {
                            ...
                            if (isPluginDescriptor)
                            {
                                ...
                                if (!((PluginDescriptor)descriptor).SupportedVersions.Contains(GSVersion.CurrentVersion))
                                    throw new Exception($"This plugin doesn't support the current version - {GSVersion.CurrentVersion}");
                            }
        ...
        protected virtual IList<IDescriptor> UploadMultipleItems(string archivePath, IList<UploadedItem> uploadedItems)
        {
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                foreach (var item in uploadedItems)
                {
                    ...
                    if (!item.SupportedVersions?.Contains(GSVersion.CurrentVersion) ?? true)
                        continue;
                    ...
                    if (item.Type == UploadedItemType.Plugin)
                        descriptorPath = $"{itemPath}{GSPluginDefaults.DescriptionFileName}";

                    if (item.Type == UploadedItemType.Theme && !string.IsNullOrEmpty(GSPluginDefaults.ThemeDescriptionFileName))
                        descriptorPath = $"{itemPath}{GSPluginDefaults.ThemeDescriptionFileName}";
                    ...
                    //ensure that the plugin current version is supported
                    if (descriptor is PluginDescriptor pluginDescriptor && !pluginDescriptor.SupportedVersions.Contains(GSVersion.CurrentVersion))
                        continue;
        ...
        public virtual IList<IDescriptor> UploadPluginsAndThemes(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var tempDirectory = _fileProvider.MapPath(GSPluginDefaults.UploadsTempPath);
                ...
                //you can find a sample of such descriptive file in Libraries\GS.Core\Plugins\Samples\
