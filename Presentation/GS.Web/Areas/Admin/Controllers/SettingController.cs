...
namespace GS.Web.Areas.Admin.Controllers
{
    public partial class SettingController : BaseAdminController
    {
        ...
        public virtual IActionResult ChangeStoreScopeConfiguration(int storeid, string returnUrl = "")
        {
            ...
            if (store != null || storeid == 0)
            {
                _genericAttributeService
                    .SaveAttribute(_workContext.CurrentCustomer, GSCustomerDefaults.AdminAreaStoreScopeConfigurationAttribute, storeid);
            }
        ...
        public virtual IActionResult ContractAttribute()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageSettings))
                return AccessDeniedView();
            var model = new ContractAttributeSettingModel();
            return View(model);
        }

        public virtual IActionResult TaskAttribute()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageSettings))
                return AccessDeniedView();
            var model = new TaskAttributeSettingModel();
            return View(model);
        }
        ...
        public virtual IActionResult ChangeEncryptionKey(GeneralCommonSettingsModel model)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(newEncryptionPrivateKey) || newEncryptionPrivateKey.Length != 16)
                    throw new GSException(_localizationService.GetResource("Admin.Configuration.Settings.GeneralCommon.EncryptionKey.TooShort"));

                var oldEncryptionPrivateKey = securitySettings.EncryptionKey;
                if (oldEncryptionPrivateKey == newEncryptionPrivateKey)
                    throw new GSException(_localizationService.GetResource("Admin.Configuration.Settings.GeneralCommon.EncryptionKey.TheSame"));
        ...
        public virtual IActionResult ToggleFullText(GeneralCommonSettingsModel model)
        {
            ...
            try
            {
                if (!_fulltextService.IsFullTextSupported())
                    throw new GSException(_localizationService.GetResource("Admin.Configuration.Settings.GeneralCommon.FullTextSettings.NotSupported"));
