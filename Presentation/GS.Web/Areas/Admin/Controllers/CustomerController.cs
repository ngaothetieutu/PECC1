...
namespace GS.Web.Areas.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        public virtual IActionResult Create(CustomerModel model, bool continueEditing)
        {
            ...
            if (newCustomerRoles.Any() && newCustomerRoles.FirstOrDefault(c => c.SystemName == GSCustomerDefaults.RegisteredRoleName) != null &&
            ...
            if (ModelState.IsValid)
            {
                ...
                if (_customerSettings.DateOfBirthEnabled)
                    _genericAttributeService.SaveAttribute(customer, GSCustomerDefaults.DateOfBirthAttribute, model.DateOfBirth);
                ...
                foreach (var customerRole in newCustomerRoles)
                {
                    ...
                    if (customerRole.SystemName == GSCustomerDefaults.AdministratorsRoleName && !_workContext.CurrentCustomer.IsAdmin())
                        continue;
                ...
                if (customer.IsVendor() && customer.VendorId == 0)
                {
                    var vendorRole = customer
                        .CustomerRoles
                        .FirstOrDefault(x => x.SystemName == GSCustomerDefaults.VendorsRoleName);
        ...
        public virtual IActionResult Edit(CustomerModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    {
                        var prevVatNumber = _genericAttributeService.GetAttribute<string>(customer, GSCustomerDefaults.VatNumberAttribute);

                        _genericAttributeService.SaveAttribute(customer, GSCustomerDefaults.VatNumberAttribute, model.VatNumber);
                        _genericAttributeService.SaveAttribute(customer,
                                 GSCustomerDefaults.VatNumberStatusIdAttribute,
                    ...
                    foreach (var customerRole in allCustomerRoles)
                    {
                        ...
                        if (customerRole.SystemName == GSCustomerDefaults.AdministratorsRoleName &&
                        ...
                        else
                        {
                            ...
                            if (customerRole.SystemName == GSCustomerDefaults.AdministratorsRoleName && !SecondAdminAccountExists(customer))
                            {
        ...
        public virtual IActionResult Impersonate(int id)
        {
            ...
            _customerService.UpdateCustomer(customer);
            _genericAttributeService.SaveAttribute<int?>(_workContext.CurrentCustomer, GSCustomerDefaults.ImpersonatedCustomerIdAttribute, customer.Id);
        ...
        public virtual IActionResult SendEmail(CustomerModel model)
        {
            ...
            try
            {
                if (string.IsNullOrWhiteSpace(customer.Email))
                    throw new GSException("Customer email is empty");
                if (!CommonHelper.IsValidEmail(customer.Email))
                    throw new GSException("Customer email is not valid");
                if (string.IsNullOrWhiteSpace(model.SendEmail.Subject))
                    throw new GSException("Email subject is empty");
                if (string.IsNullOrWhiteSpace(model.SendEmail.Body))
                    throw new GSException("Email body is empty");

                var emailAccount = _emailAccountService.GetEmailAccountById(_emailAccountSettings.DefaultEmailAccountId);
                if (emailAccount == null)
                    emailAccount = _emailAccountService.GetAllEmailAccounts().FirstOrDefault();
                if (emailAccount == null)
                    throw new GSException("Email account can't be loaded");
        ...
        public virtual IActionResult SendPm(CustomerModel model)
        {
            ...
            try
            {
                if (!_forumSettings.AllowPrivateMessages)
                    throw new GSException("Private messages are disabled");
                if (customer.IsGuest())
                    throw new GSException("Customer should be registered");
                if (string.IsNullOrWhiteSpace(model.SendPm.Subject))
                    throw new GSException("PM subject is empty");
                if (string.IsNullOrWhiteSpace(model.SendPm.Message))
                    throw new GSException("PM message is empty");
        ...
        public virtual IActionResult LoadCustomerStatistics(string period)
        {
            ...
            var searchCustomerRoleIds = new[] { _customerService.GetCustomerRoleBySystemName(GSCustomerDefaults.RegisteredRoleName).Id };
