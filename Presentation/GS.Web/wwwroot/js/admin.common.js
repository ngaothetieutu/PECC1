...
function LoadContractResourceInfo(contractId) {
    $.ajax({
        cache: false,
        type: "GET",
        dataType: "json",
        url: "/Contract/_ContractResources?ContractId=" + contractId,
        complete: function (data) {
            var _template = kendo.template($("#gsContractResourceTemplate").html());
            var strtemp = "";
            $.each(data.responseJSON, function (index, val) {
                strtemp = strtemp + _template(val);
            });
            $("#divContractResources_" + contractId).html(strtemp);
        },
        traditional: true
    });
}
